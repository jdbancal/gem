...
if length(indices) == 1
    ...
    elseif max(indices{1}) > prod(s)
        ...
        if s(1) == 1
            sgem_mex('resize', this.objectIdentifier, 1, max(indices{1}));
        else
            sgem_mex('resize', this.objectIdentifier, max(indices{1}), 1);
        end
    end
    ...
    if (max(indices{1}) > s(1)) || (max(indices{2}) > s(2))
        ...
        sgem_mex('resize', this.objectIdentifier, max(s(1), max(indices{1})), max(s(2), max(indices{2})));
    end
...
result = copy(this);
if length(indices) == 1
    sgem_mex('subsasgn', result.objectIdentifier, values.objectIdentifier, indices{1}-1);
else
    sgem_mex('subsasgn', result.objectIdentifier, values.objectIdentifier, indices{1}-1, indices{2}-1);
end
