...
function [Y I] = min(this, varargin)
    ...
    if length(varargin) ~= 1
        ...
        if (isempty(varargin) && (size1(1) > 1)) || ((length(varargin) == 2) && (varargin{2} == 1))
            [newObjectIdentifier I] = sgem_mex('colMin', this.objectIdentifier);
            I = I'+1;
        else
            [newObjectIdentifier I] = sgem_mex('rowMin', this.objectIdentifier);
            I = I+1;
        end
        ...
        if ~issparse(this)
            ...
            % new object with the result, we keep the corresponding handle...
            newObjectIdentifier = gem_mex('ewMin', objectIdentifier(this), objectIdentifier(varargin{1}));
            % ...  and create a new matlab object to keep this handle
            Y = gem('encapsulate', newObjectIdentifier);
        else
            % Now we call the element-wise minimum procedure. Since the function creates a
            % new object with the result, we keep the corresponding handle...
            newObjectIdentifier = sgem_mex('ewMin', objectIdentifier(this), objectIdentifier(varargin{1}));
            % ...  and create a new matlab object to keep this handle
            Y = sgem('encapsulate', newObjectIdentifier);
        end
