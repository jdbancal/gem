...
% because the null element '0' is not preserved upon addition by a non-zero
% number.
% 
...
%
% However, matlab does not always encode results which are expected to be 
% sparse in sparse matrices, and results expected to be full in full matrices.
% For instance, exp(sparse([0 1 0])) returns by default a sparse result, even
% though 0 is not preserved by the exponentiation : exp(0) = 1. This
% typically results in a full matrix being encoded as a sparse one (which is
% inefficient)
% 
% This function allows one to define whether we want the gem library to produce
% sparse or full matrices based on whether the applied function preserves the
% null element, or whether we want to choose matrix types the way matlab does.
%
% To get the current setting, use
%   precision = gemSparseLikeMatlab
