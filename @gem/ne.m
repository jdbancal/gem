...
function result = ne(this, varargin)
    ...
    if isequal(class(this), 'gem') && isequal(class(varargin{1}), 'gem')
        result = logical(gem_mex('ne', this.objectIdentifier, varargin{1}.objectIdentifier));
    else
        result = logical(sgem_mex('ne', this.objectIdentifier, varargin{1}.objectIdentifier));
    end
