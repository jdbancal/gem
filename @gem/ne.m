...
function result = ne(this, varargin)
    ...
    if isequal(class(this), 'gem') && isequal(class(varargin{1}), 'gem')
        result = logical(gem_mex('ne', objectIdentifier(this), objectIdentifier(varargin{1})));
    else
        result = logical(sgem_mex('ne', objectIdentifier(this), objectIdentifier(varargin{1})));
    end
